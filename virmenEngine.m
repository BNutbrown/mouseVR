...
while ~vr.experimentEnded
    ...
    %daq frame flip output
    if ~vr.paused
        vr.frameflip = ~vr.frameflip;  % flip the value
        if strcmpi(vr.ComputerName, 'WINDOWS-9AEMBGN')  % rig pc
            fwrite(vr.tcp.t, vr.tcp.FRAME)
        else
            outputSingleScan(vr.daqframeTTL.daqSessFrame, vr.frameflip);  % write the new value
        end
    end
    ...
    if vr.iterations == 1
        ...
        targetFrameTime = 0.05;  % 30ms
        if vr.dt < targetFrameTime
%             pause(targetFrameTime - vr.dt)
            java.lang.Thread.sleep((targetFrameTime - vr.dt)*1000)  % more accurate than matlab builtin
        end
        vr.timeElapsed = toc(firstTic);
    end
...
%             vr.movementType = 'velocity';
%         else
%             [vr.movement, vr.movementType, vr.RotaryEncoder.count] = vr.exper.movementFunction(vr);
            vr = vr.exper.movementFunction(vr);
%             keyboard
%         end
    catch ME
        drawnow;
